...
def create_npy_dataset_from_midi(folder, max_polyphony, dataset_name, ins_ids='all', scan_subfolders=True, dedup=False, max_idx=None, out_dir: str=None, filter: bool=True):
    if out_dir is None:
        out_dir= 'data'
    ...
    torch.save(torch.cat(results_data, dim=0), f'{out_dir}/{dataset_name}.pt')
    torch.save(torch.cat(results_shift, dim=0), f'{out_dir}/{dataset_name}.pitch_shift_range.pt')
    lengths = [len(data) for data in results_data]
    # save midi file names
    f = open(f'{out_dir}/{dataset_name}.txt', 'w')
    for i, midi_file in enumerate(midi_files):
        f.write(str(i) + '\t' + midi_file + '\n')
    # np.save(f'data/{dataset_name}.length.npy', np.array(lengths))
    torch.save(torch.tensor(lengths), f'{out_dir}/{dataset_name}.length.pt')
